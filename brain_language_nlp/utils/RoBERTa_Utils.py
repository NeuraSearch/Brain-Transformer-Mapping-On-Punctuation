...
def get_roberta_layer_representations(seq_len, text_array, remove_chars, word_ind_to_extract,feature_strategy,home_path):
    ...
    for word in text_array:
        current_token_embedding = get_bertlike_token_embeddings([word], tokenizer, model, remove_chars,feature_strategy,home_path)
        token_embeddings.append(np.mean(current_token_embedding.detach().numpy(), 1))

    # where to store layer-wise albert embeddings of particular length
    albert = {}
    for layer in range(12):
        albert[layer] = []
    albert[-1] = token_embeddings
    ...
    for _ in range(seq_len):
        albert = add_avrg_token_embedding_for_specific_word(word_seq,
                                                          tokenizer,
                                                          model,
                                                          remove_chars,
                                                          from_start_word_ind_to_extract,
                                                          albert,feature_strategy,home_path)
...
def predict_bertlike_embeddings(words_in_array, tokenizer, model, remove_chars, feature_strategy, home_path):
    # print(words_in_array)
    special_characters = []
    if "cross" in feature_strategy:
        special_characters.append("+")
    else:
        with open(home_path + "/data/special_characters.json", "r") as json_file:
            data = json.load(json_file)
            special_characters = list(data.keys())[:-1]
            if "everything" in feature_strategy:
                special_characters.append(".")
                special_characters.append("?")
    ...
    for i, word in enumerate(words_in_array):
        ...
        for token in word_tokens:
            if token not in remove_chars:  # don't add any tokens that are in remove_chars
                if token in special_characters and "normal" not in feature_strategy:
                    if "removing" not in feature_strategy:
                        if "padding" in feature_strategy:
                            token = "[PAD]"
    ...
    inputs = tokenizer.encode_plus(seq_tokens, return_tensors="pt", add_special_tokens=False,is_split_into_words=True)
    encoded_layers = model(**inputs)
    pooled_output = np.squeeze(encoded_layers.pooler_output.detach().numpy())
    return encoded_layers.hidden_states[1:], word_ind_to_token_ind, pooled_output


# add the embeddings for a specific word in the sequence
# token_inds_to_avrg: indices of tokens in embeddings output to avrg
def add_word_bertlike_embedding(bert_dict, embeddings_to_add, token_inds_to_avrg, specific_layer=-1):
    if specific_layer >= 0:  # only add embeddings for one specified layer
        ...
        for layer, layer_embedding in enumerate(embeddings_to_add):
            full_sequence_embedding = layer_embedding.detach().numpy()
            toAppend = np.mean(full_sequence_embedding[0], 0)
            bert_dict[layer].append(toAppend)  # avrg over all tokens for specified word
    return bert_dict
...
def add_avrg_token_embedding_for_specific_word(word_seq, tokenizer, model, remove_chars, from_start_word_ind_to_extract,
                                               bert_dict,feature_strategy,home_path):
    word_seq = ['<s>'] + list(word_seq) + ['</s>']
    all_sequence_embeddings, word_ind_to_token_ind, _ = predict_bertlike_embeddings(word_seq, tokenizer, model,
                                                                                    remove_chars,feature_strategy,home_path)
    token_inds_to_avrg = word_ind_to_token_ind[from_start_word_ind_to_extract]
    bert_dict = add_word_bertlike_embedding(bert_dict, all_sequence_embeddings, token_inds_to_avrg)
...
# get the BERT token embeddings
def get_bertlike_token_embeddings(words_in_array, tokenizer, model, remove_chars, feature_strategy, home_path):
    for word in words_in_array:
        if word in remove_chars:
            print(
                'An input word is also in remove_chars. This word will be removed and may lead to misalignment. Proceed with caution.')
    ...
    for i, word in enumerate(words_in_array):
        ...
        if word in ['<s>', '</s>']:  # [CLS] and [SEP] are already tokenized
            word_tokens = [word]
        else:
...
def add_all_bertlike_embeddings(bert_dict, embeddings_to_add, specific_layer=-1):
    if specific_layer >= 0:
        layer_embedding = embeddings_to_add[specific_layer]
...
def add_last_nonsep_bertlike_embedding(bert_dict, embeddings_to_add, specific_layer=-1):
    if specific_layer >= 0:
        layer_embedding = embeddings_to_add[specific_layer]
        full_sequence_embedding = layer_embedding.detach().numpy()
...
def add_cls_bertlike_embedding(bert_dict, embeddings_to_add, specific_layer=-1):
    if specific_layer >= 0:
        layer_embedding = embeddings_to_add[specific_layer]
        full_sequence_embedding = layer_embedding.detach().numpy()
