...
def get_albert_layer_representations(seq_len, text_array, remove_chars, word_ind_to_extract):
    ...
    for word in text_array:
        current_token_embedding = get_bertlike_token_embeddings([word], tokenizer, model, remove_chars)
...
def predict_bertlike_embeddings(words_in_array, tokenizer, model, remove_chars):
    ...
    sequence_string = ' '.join(words_in_array)
    inputs = tokenizer(sequence_string, return_tensors="pt",add_special_tokens=False)
...
def get_bertlike_token_embeddings(words_in_array, tokenizer, model, remove_chars):
    for word in words_in_array:
        if word in remove_chars:
            print(
                'An input word is also in remove_chars. This word will be removed and may lead to misalignment. Proceed with caution.')
